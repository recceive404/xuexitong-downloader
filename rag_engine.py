...
用中文回答，简洁清晰。"""
        ...
        try:
            ...
            if resp.status_code == 200:
                raw = resp.text
                # DeepSeek 返回可能不是标准 JSON，尝试解析
                try:
                    data = resp.json()
                except:
                    data = raw
                # 调试：打印类型
                print(f"    [DEBUG] resp type={type(data).__name__}, len={len(data) if hasattr(data,'__len__') else 'N/A'}")
                if isinstance(data, str):
                    # 尝试用 ast.literal_eval 解析 Python 字面量
                    import ast
                    try:
                        data = ast.literal_eval(data)
                        print(f"    [DEBUG] parsed with ast, type={type(data).__name__}")
                    except:
                        pass
                # DeepSeek 返回格式：列表 [{type:'thinking',...}, {type:'text',...}]
                if isinstance(data, list):
                    for item in data:
                        if isinstance(item, dict) and item.get("type") == "text":
                            return item.get("text", "")
                    return f"[DEBUG] list without text item: {str(data)[:300]}"
                # OpenAI 兼容格式
                ...
                return f"[DEBUG] unknown format (type={type(data).__name__}): {str(data)[:500]}"
            else:
                # 降级：返回检索结果
                return (
